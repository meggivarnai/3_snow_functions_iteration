...
#add titles and such
ggplot(by_year, aes(x=month, y=monthly_temp, color=site))+
  geom_line()

#we want functions to pull from raw datasets!
monthly_plots <- function(weather_data, year_of_interest){
  by_year<- weather_data%>%
  filter(year==year_of_interest) %>%
  group_by(month,site) %>%
  summarise(monthly_temp=mean(air.temp..K.)) %>%
  print(ggplot(aes(x=month, y=monthly_temp, color=site))+
  geom_line())
  
}
                          
monthly_plots(weather_data,year_of_interest)

...
weatherfiles<-filenames

emptydata<-list() #create an empty list to load data into
weatherdata <- for (i in 1:length(weatherfiles)){
  emptydata[[i]]<-read_delim(filenames) %>%
    select(Year, DOY)
}



full_data<- rbind(headers,)
